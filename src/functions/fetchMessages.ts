...
async function pushMessagesToArray(connection: Connection, arr: IRawInfo[], messagesArray: Message[], threadInfo?: threadInfo): Promise<IRawInfo[]> {
...
async function fetchMessages(
    ...
    while (fetchedMessages.size > 0) {
        const boundaryMessage = fetchDirection === 'before'
        ...
        if (!boundaryMessage || (period && boundaryMessage.createdAt < period)) {
            ...
            channel instanceof ThreadChannel ? await pushMessagesToArray(connection, messagesToStore, [...fetchedMessages.values()], { threadId: channel.id, threadName: channel.name, channelId: channel.parent?.id, channelName: channel.parent?.name }) : await pushMessagesToArray(connection, messagesToStore, [...fetchedMessages.values()])
            break;
        }
...
export default async function fetchChannelMessages(connection: Connection, channel: TextChannel, period: Date) {
    ...
    const oldestChannelRawInfo = await rawInfoService.getOldestRawInfo(connection, { channelId: channel?.id, threadId: null });
    const newestChannelRawInfo = await rawInfoService.getNewestRawInfo(connection, { channelId: channel?.id, threadId: null });
    ...
    for (const thread of threads) {
        const oldestThreadRawInfo = await rawInfoService.getOldestRawInfo(connection, { channelId: channel?.id, threadId: thread.id });
        const newestThreadRawInfo = await rawInfoService.getNewestRawInfo(connection, { channelId: channel?.id, threadId: thread.id });
