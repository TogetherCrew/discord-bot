...
import { ChatInputCommandInteraction_broker, InteractionResponse, InteractionResponseEditData } from '../interfaces/Hivemind.interface';
 ...
 * @returns {Promise<IDiscordOathBotCallback>}
 */
async function editOriginalInteractionResponse(interaction: ChatInputCommandInteraction_broker, data: InteractionResponseEditData) {
    try {
        const response = await fetch(`https://discord.com/api/webhooks/${interaction.applicationId}/${interaction.token}/messages/@original`, {
            method: 'PATCH',
