...
const logger = logger_1.default.child({ module: 'GuildMemberService' });
/**
 ...
 */
async function createGuildMember(connection, guildMember) {
    try {
        ...
        if (error.code == 11000) {
            logger.warn({ database: connection.name, guild_member_id: guildMember.discordId }, 'Failed to create duplicate guild member');
            return null;
        }
        logger.error({ database: connection.name, guild_member_id: guildMember.discordId, error }, 'Failed to create guild member');
        return null;
    }
}
/**
 ...
 */
async function createGuildMembers(connection, guildMembers) {
    try {
 ...
 */
async function getGuildMember(connection, filter) {
 ...
 */
async function getGuildMembers(connection, filter) {
 ...
 */
async function updateGuildMember(connection, filter, updateBody) {
    try {
 ...
 */
async function updateGuildMembers(connection, filter, updateBody) {
    try {
 ...
 */
async function deleteGuildMember(connection, filter) {
    try {
 ...
 */
async function deleteGuildMembers(connection, filter) {
    try {
 ...
 *
 */
async function handelGuildMemberChanges(connection, guildMember) {
    ...
    try {
        ...
        if (!guildMemberDoc) {
            await createGuildMember(connection, Object.assign(Object.assign({}, commonFields), { isBot: guildMember.user.bot }));
        }
    }
    catch (error) {
        logger.error({ guild_id: connection.name, guild_member_id: guildMember.id, error }, 'Failed to handle guild member changes');
    }
}
/**
 ...
 */
function getNeededDateFromGuildMember(guildMember) {
    return {
